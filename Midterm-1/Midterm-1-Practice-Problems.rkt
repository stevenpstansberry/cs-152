...
;;; Function remove-all: element list -> list 
;;; return a new list obtained by removing all the occurrences ;;; 
;;;of the element from the original list.
  
  
(define (remove-all x xs)
  (cond ((null? xs) '())
  ((equal? x (car xs)) (remove-all x (cdr xs))) ;ignore car xs
  (else (cons (car xs) (remove-all x (cdr xs)))))) ; add car xs since it is not the same
